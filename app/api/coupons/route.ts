...
//     // Only add .not(...in...) if there are used coupons
//     if (usedCouponIds.length > 0) {
//       query = query.not("id", "in", usedCouponIds)
//     }
...
//     return NextResponse.json({ coupons: filtered }, { status: 200 })
//   } catch (error) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: usedCoupons, error: usedError } = await supabase
      ...
      console.log('allusedcop',usedCoupons)
